...
public class Translator {
    ...
    private void exprlist(int operation) {                                             //exprlist prende ad argomento l'operazione da eseguire
        switch(look.tag){                                                              //questo serve infatti a tenere l'operazione in memoria 
            case '+':   //EL -->  E EL' --- Guida = +,-,*,/,NUM,ID                    //nel caso di somme o moltiplicazioni al fine di poterle concatenare
            case '-':
            case '*':
            case '/':
            case Tag.NUM:           
                ...
                }                       //da notare che in caso di operando print non emettiamo nulla. Se ne occupera la procedura stat
                break;
            case Tag.ID:
                if (look.tag==Tag.ID) {
                    int id_addr = st.lookupAddress(((Word)look).lexeme);
